(() => {
    ...
    refs.header_openMenuBtn.addEventListener('click', toggleModal);
    refs.header_closeMenuBtn.addEventListener('click', toggleModal);

    var links = document.getElementsByClassName('pop-up-menu__navigation__element');
        for (var i = 0; i < links.length; i++) {
            links[i].addEventListener('click', toggleModal);
    }
